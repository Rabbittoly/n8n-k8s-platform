...
if [ "$CONFIGURE_RESOURCES" = "true" ]; then
  ...
  # n8n resources
  echo "n8n resources:"
  prompt_with_default "  CPU limit (cores)" "1" "N8N_CPU_LIMIT"
  prompt_with_default "  Memory limit (Mi)" "2048" "N8N_MEMORY_LIMIT"
  prompt_with_default "  CPU request (cores)" "500m" "N8N_CPU_REQUEST"
  prompt_with_default "  Memory request (Mi)" "1024" "N8N_MEMORY_REQUEST"
  
  # PostgreSQL resources
  echo
  echo "PostgreSQL resources:"
  prompt_with_default "  CPU limit (cores)" "1" "PG_CPU_LIMIT"
  prompt_with_default "  Memory limit (Mi)" "2048" "PG_MEMORY_LIMIT"
  prompt_with_default "  CPU request (cores)" "500m" "PG_CPU_REQUEST"
  prompt_with_default "  Memory request (Mi)" "1024" "PG_MEMORY_REQUEST"
  
  # Redis resources
  echo
  echo "Redis resources:"
  prompt_with_default "  CPU limit (cores)" "500m" "REDIS_CPU_LIMIT"
  prompt_with_default "  Memory limit (Mi)" "1024" "REDIS_MEMORY_LIMIT"
  prompt_with_default "  CPU request (cores)" "250m" "REDIS_CPU_REQUEST"
  prompt_with_default "  Memory request (Mi)" "512" "REDIS_MEMORY_REQUEST"
fi
...
# Replicas
prompt_with_default "Number of n8n replicas" "2" "N8N_REPLICAS"
...
# Monitoring
prompt_yes_no "Install monitoring (Prometheus/Grafana)?" "y" "INSTALL_MONITORING"
